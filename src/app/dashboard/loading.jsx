export default function DashboardLoading() {
  return (
    <div className="mx-auto w-full max-w-5xl animate-pulse px-6 py-10">
      {/* Header Skeleton — matches: title + subtitle + badge */}
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-2">
          <div className="h-8 w-40 rounded-xl bg-[#EAE0D3]/80 dark:bg-[#3A332A]" />
          <div className="h-4 w-60 rounded-lg bg-[#EAE0D3]/60 dark:bg-[#3A332A]/60" />
        </div>
        <div className="h-7 w-28 rounded-full bg-[#EAE0D3]/70 dark:bg-[#3A332A]/70" />
      </div>

      {/* 4 Stat Cards Skeleton — matches: label + icon row, then value */}
